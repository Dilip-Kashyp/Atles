...
def _generate_with_fallback(prompt: str, fallback_value: str) -> str:
    ...
    try:
        ...
        return resp.text or fallback_value
    except Exception as exc:
        log.warning("[CHECKPOINT: RESPONSE_FORMATTER_FALLBACK] Small LLM failed: %s", exc)
...
async def format_error(exc: Exception) -> str:
    ...
    if "429" in exc_str and "RESOURCE_EXHAUSTED" in exc_str:
        fallback = "*Oops! I've run out of AI energy for now.* (My API quota is exhausted). Let me rest a bit, or please upgrade my API billing plan!"
    elif "401" in exc_str or "403" in exc_str:
        fallback = "*I'm having trouble authenticating* with one of my tools. Please check my API keys in the server config."
    else:
        fallback = "*I hit an unexpected snag while thinking.* (Check the server logs for the technical details)."
...
async def format_response(response: str) -> str:
    ...
    return _generate_with_fallback(prompt, response)
