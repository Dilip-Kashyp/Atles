...
async def format_error(exc: Exception) -> str:
    """Uses a small LLM to format technical errors into natural, friendly language."""
    exc_str = str(exc)
    ...
    try:
        ...
        prompt = (
            "You are a helpful AI bot. You just encountered this tech   nical error while trying to help a user. Don't use any emoji's in the response."
            f"Explain to the user what went wrong in a natural, friendly, and non-technical way (1-2 sentences max). Error: {exc_str}" 
        )
        
        # Use the fastest/cheapest model to format the error
        ...
        # If the fallback LLM ALSO fails (e.g. global account rate limit), use hardcoded text
...
async def format_response(response: str) -> str:
    """Uses a small LLM to format standard responses for cross-platform consistency."""
    settings = get_settings()
